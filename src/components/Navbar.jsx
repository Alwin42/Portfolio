  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b-2 border-black font-body">
      <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        {/* Logo */}
        <Link to="/"
          className="font-display text-2xl tracking-widest text-[#1D3557] hover:text-[#E63946] transition-colors">
          ALWIN.DEV
        </Link>
      ...
      {menuOpen && (
        <div className="md:hidden border-t-2 border-black bg-white">
          {links.map(({ path, label }) => (
            <Link
              ...
              className={`block px-6 py-4 text-xs font-bold uppercase tracking-widest border-b border-black/10
                ${pathname === path ? 'bg-[#F1C40F] text-black' : 'hover:bg-[#F1FAEE]'}`}
            >
              {label}
            </Link>
          ))}
          <a
            ...
            className="block px-6 py-4 text-xs font-bold uppercase tracking-widest border-b border-black/10 hover:bg-[#F1FAEE] cursor-pointer"
          >
            About
          </a>
        </div>
